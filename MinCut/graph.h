...
private:
	...
	long		TIME;							// monotonically（单调的） increasing global counter
	flowtype	flow;							// total flow
	int			maxflow_iteration;				// counter
		
									
	DBlock<nodeptr>		*nodeptr_block;
	Block<node_id>		*changed_list;// reusing trees & list of changed pixels
...
template <typename captype, typename tcaptype, typename flowtype> //初始节点数目和边的数目都是16，见构造函数
...
template <typename captype, typename tcaptype, typename flowtype> //t-link初始化
...
template <typename captype, typename tcaptype, typename flowtype> //n-link初始化
	inline void Graph<captype,tcaptype,flowtype>::add_edge(node_id _i, node_id _j, captype cap, captype rev_cap)
{
	...
	a -> sister = a_rev;
	a_rev -> sister = a;
	//类似栈的功能，同一个源节点，最先初始化的边的next=NULL;最先初始化的边成为了下一个相同源节点的边的next
	a -> next = i -> first;
	i -> first = a;
	a_rev -> next = j -> first;
	j -> first = a_rev;
...
template <typename captype, typename tcaptype, typename flowtype> //返回第一条边的指针
	inline typename Graph<captype,tcaptype,flowtype>::arc* Graph<captype,tcaptype,flowtype>::get_first_arc()
{
	return arcs;
}

template <typename captype, typename tcaptype, typename flowtype> //返回给定边的下一条边的指针
	inline typename Graph<captype,tcaptype,flowtype>::arc* Graph<captype,tcaptype,flowtype>::get_next_arc(arc* a) 
{
	return a + 1; 
}

template <typename captype, typename tcaptype, typename flowtype> //返回特定边的两个节点i->j
...
template <typename captype, typename tcaptype, typename flowtype> //得到节点的残存容量
...
template <typename captype, typename tcaptype, typename flowtype> //得到边的残存容量
...
template <typename captype, typename tcaptype, typename flowtype> //设置节点的残存容量
...
template <typename captype, typename tcaptype, typename flowtype> //设置边的残存容量
...
template <typename captype, typename tcaptype, typename flowtype> //最大流求解后，确定节点属于哪一个标签
