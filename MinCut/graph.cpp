	...
	void Graph<captype,tcaptype,flowtype>::reallocate_nodes(int num)
{
	...
	nodes = (node*) realloc(nodes_old, node_num_max*sizeof(node));//改变内存的大小。
	if (!nodes) { if (error_function) (*error_function)("Not enough memory!"); exit(1); }

	node_last = nodes + node_num;
	node_max = nodes + node_num_max;
	//当前指针后面没有足够的内存空间，则去别的地方分配一块完整的空间，将数据拷贝过去
	//下面更新了数据之间的指针关系
	if (nodes != nodes_old)
	{
		node* i;
	...
	*/
	...
	inline void Graph<captype, tcaptype, flowtype>::set_active(node *i)
	{
		if (!i->next)
		{
			...
			if (queue_last[1]) queue_last[1]->next = i;//前一个active的节点的next为当前的节点i
			else               queue_first[1] = i;
			queue_last[1] = i;
			i->next = i;//先假设节点的next为自己
		}
	}
	...
	*/
	...
	inline typename Graph<captype, tcaptype, flowtype>::node* Graph<captype, tcaptype, flowtype>::next_active()
	{
		...
		while (1)
		{
			...
			else              queue_first[0] = i->next;//删除next是自己的节点，将next不是自己的节点的next赋值给queue_first[0]
			i->next = NULL;
		...
		while (1)
		{
			...
			if (!i->is_sink)
			{
				...
				for (a = i->first; a; a = a->next)
					...
					{
						j = a->head;
						if (!j->parent)//j不与两个终端节点相连，即j是自由节点
						{
							j->is_sink = 0;
							j->parent = a->sister;
							j->TS = i->TS;
						...
						else if (j->is_sink)//节点j是否是sink树的节点，若是，则找到了一条增光路径。由于is_sink在类中定义的时候限制了位，本来是1的值显示出来是-1，此处需要注意。
							break;
			...
			if (a)
			{
				...
				/* augmentation *////////////////////////////////////////////////
				augment(a);//找到增广路径中的瓶颈流，将整个增光路径上的流减少相应的量
						   /* augmentation end *///////////////////////////////////////////
